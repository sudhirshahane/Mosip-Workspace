...
public class SyncAuthTokenServiceImpl {
    ...
    public String getAuthToken(String requestData) {
        ...
        if(parts.length == 3) {
            byte[] header = CryptoUtil.decodeURLSafeBase64(parts[0]);
            byte[] payload = CryptoUtil.decodeURLSafeBase64(parts[1]);
            byte[] signature = CryptoUtil.decodeURLSafeBase64(parts[2]);
...
//                return CryptoUtil.encodeToURLSafeBase64(cipher);
    ...
    private Machine validateRequestData(byte[] header, byte[] payload, byte[] signature) {
        ...
        String keyIndex="eyJraWQiIDogImUwOjg3OmJjOjMxOjBlOjQ3OjdiOmZlOjUwOmU3OmZhOmVlOjRmOjI1OjIyOjNjOjQ5OmQ5OjVhOmZlOjZlOmE2Om";

        if(Objects.nonNull(jwtheader.getKeyId())) {
//            List<Machine> machines = machineRepository.findBySignKeyIndex(jwtheader.getKeyId());
            List<Machine> machines = machineRepository.findBySignKeyIndex(keyIndex);
