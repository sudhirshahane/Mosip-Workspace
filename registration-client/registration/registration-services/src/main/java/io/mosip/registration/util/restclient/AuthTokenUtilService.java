...
public class AuthTokenUtilService {
	...
	public void init() {
		...
		backOffPolicy.setBackOffPeriod(
				(Long) ApplicationContext.map().getOrDefault("mosip.registration.retry.delay.auth", 1000l));

		SimpleRetryPolicy retryPolicy = new SimpleRetryPolicy();
		retryPolicy.setMaxAttempts(
				(Integer) ApplicationContext.map().getOrDefault("mosip.registration.retry.maxattempts.auth", 2));

		retryTemplate = new RetryTemplateBuilder().retryOn(ConnectionException.class).customPolicy(retryPolicy)
				.customBackoff(backOffPolicy).build();
	}

	public boolean hasAnyValidToken() {
		UserToken userToken = userTokenRepository
				.findTopByTokenExpiryGreaterThanAndUserDetailIsActiveTrueOrderByTokenExpiryDesc(
						System.currentTimeMillis() / 1000);
		if (userToken != null) {
			return true;
		}
		userToken = userTokenRepository
				.findTopByRtokenExpiryGreaterThanAndUserDetailIsActiveTrueOrderByRtokenExpiryDesc(
						System.currentTimeMillis() / 1000);
		if (userToken != null) {
			return true;
		}
	...
	private String refreshAuthToken(String userId, String refreshToken) throws RegBaseCheckedException {
		...
		try {
			...
			String header = String.format("{\"kid\" : \"%s\"}",
					CryptoUtil.computeFingerPrint(clientCryptoFacade.getClientSecurity().getSigningPublicPart(), null));
			...
			setURI(requestHTTPDTO, new HashMap<>(),
					getEnvironmentProperty("auth_by_password", RegistrationConstants.SERVICE_URL));
			...
			userDetailDAO.updateAuthTokens(userId, jsonObject.getString("token"), jsonObject.getString("refreshToken"),
	...
	public AuthTokenDTO getAuthTokenAndRefreshToken(LoginMode loginMode, LoginUserDTO loginUserDTO)
		...
		try {
			...
			case PASSWORD:
				payload = String.format(
						"{\"userId\" : \"%s\", \"password\": \"%s\", \"authType\":\"%s\", \"timestamp\" : \"%s\"}",
						loginUserDTO.getUserId(), loginUserDTO.getPassword(), "NEW", timestamp);
				break;
			case OTP:
				payload = String.format(
						"{\"userId\" : \"%s\", \"otp\": \"%s\", \"authType\":\"%s\", \"timestamp\" : \"%s\"}",
			...
			setURI(requestHTTPDTO, new HashMap<>(),
					getEnvironmentProperty("auth_by_password", RegistrationConstants.SERVICE_URL));
			Map<String, Object> responseMap = restClientUtil.invokeForToken(requestHTTPDTO);
			Map<String, Object> respBody = (Map<String, Object>) responseMap
					.get(RegistrationConstants.REST_RESPONSE_BODY);
//            JSONObject jsonObject = getAuthTokenResponse(responseMap);
			JSONObject jsonObject = new JSONObject(new String((String) respBody.get("response")));
			AuthTokenDTO authTokenDTO = new AuthTokenDTO();
	...
	public String sendOTP(String userId) throws ConnectionException {
		...
		try {
			...
			jsonObject.put("otpChannel", new JSONArray(ApplicationContext.map().get(RegistrationConstants.OTP_CHANNELS)
			...
			setTimeout(requestHTTPDTO);
			setURI(requestHTTPDTO, new HashMap<>(),
					getEnvironmentProperty("auth_by_otp", RegistrationConstants.SERVICE_URL));
			Map<String, Object> responseMap = restClientUtil.invokeForToken(requestHTTPDTO);
			...
			if (responseMap.get(RegistrationConstants.RESPONSE) != null) {
				LinkedHashMap<String, String> otpMessage = (LinkedHashMap<String, String>) responseMap.get("response");
			...
			if (responseMap.get(RegistrationConstants.ERRORS) != null) {
				String errMsg = ((List<LinkedHashMap<String, String>>) responseMap.get(RegistrationConstants.ERRORS))
	...
	private JSONObject getAuthTokenResponse(Map<String, Object> responseMap) throws RegBaseCheckedException {
		if (responseMap.get(RegistrationConstants.REST_RESPONSE_BODY) != null) {
			...
			if (respBody.get("response") != null) {
				byte[] decryptedData = clientCryptoFacade
						.decrypt(CryptoUtil.decodeURLSafeBase64((String) respBody.get("response")));
	...
	private void updateUserDetails(@NotNull String userId, String password, String token, String refreshToken)
			throws Exception {
//		if (password != null)
//			userDetailDAO.updateUserPwd(userId, password);
	...
	private List<String> getRoles(@NotNull DecodedJWT decodedJWT) {
		String realmAccess = decodedJWT.getClaim(ROLES).asString().toString();
		List<String> rolesList = Arrays.asList(realmAccess.split(","));
		return rolesList;
//				(!realmAccess.isNull()) ? (List<String>) realmAccess: new ArrayList<>();
	}

	private String getUsername(@NotNull DecodedJWT decodedJWT) {
		return decodedJWT.getSubject() == null ? null : decodedJWT.getSubject().toString().trim();
	}
}
