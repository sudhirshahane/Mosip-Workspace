...
public class BaseService {
	...
	public List<String> getMandatoryLanguages() {
		return mandatoryLanguages.stream().filter(item -> !item.isBlank()).map(String::strip).map(String::toLowerCase)
	...
	public List<String> getOptionalLanguages() throws PreConditionCheckException {
		...
		List<String> optionalLang = optionalLanguages.stream().filter(item -> !item.isBlank()).map(String::strip)
	...
	public int getMinLanguagesCount() {
		...
		// min-count is 0 / less than 0, then set to mandatory list size
				...
				// check if its greater than mandatory list size then set it back to mandatory
				// list size
	...
	public int getMaxLanguagesCount() throws PreConditionCheckException {
		...
		// max-count is (0 / less than 0) OR ( greater than mandatory + optional size),
		// set to mandatory + optional size
	...
	public String getCenterId() {
		...
		if (machineMaster != null && machineMaster.getRegCenterId() != null) {
			Optional<RegistrationCenter> result = registrationCenterRepository
					.findByIsActiveTrueAndRegistartionCenterIdIdAndRegistartionCenterIdLangCode(
	...
	public BIR buildBir(String bioAttribute, long qualityScore, byte[] iso, ProcessedLevelType processedLevelType) {
		...
		return new BIR.BIRBuilder().withBdb(iso).withVersion(new VersionType(1, 1))
	...
	public BIR buildBir(BiometricsDto biometricsDto) {
		...
		return new BIR.BIRBuilder().withBdb(biometricsDto.getAttributeISO()).withVersion(new VersionType(1, 1))
				.withCbeffversion(new VersionType(1, 1))
				.withBirInfo(new BIRInfo.BIRInfoBuilder().withIntegrity(false).build())
				.withBdbInfo(new BDBInfo.BDBInfoBuilder().withFormat(birFormat).withQuality(qualityType)
						.withType(Arrays.asList(biometricType))
						.withSubtype(getSubTypes(biometricType, biometricsDto.getBioAttribute()))
	...
	public void commonPreConditionChecks(String action) throws PreConditionCheckException {
		...
		if (SessionContext.isSessionContextAvailable() && !userDetailService.isValidUser(SessionContext.userId())
				&& !isInitialSync())
	...
	public BufferedImage concatImages(byte[] image1, byte[] image2, byte[] image3, byte[] image4, String imagePath) {
		try {
			BufferedImage img1 = ImageIO
					.read((image1 == null || image1.length == 0) ? this.getClass().getResourceAsStream(imagePath)
							: new ByteArrayInputStream(image1));
			BufferedImage img2 = ImageIO
					.read((image2 == null || image2.length == 0) ? this.getClass().getResourceAsStream(imagePath)
							: new ByteArrayInputStream(image2));
			BufferedImage img3 = ImageIO
					.read((image3 == null || image3.length == 0) ? this.getClass().getResourceAsStream(imagePath)
							: new ByteArrayInputStream(image3));
			BufferedImage img4 = ImageIO
					.read((image4 == null || image4.length == 0) ? this.getClass().getResourceAsStream(imagePath)
			...
			int height = Math.max(Math.max(img1.getHeight(), img2.getHeight()),
					Math.max(img3.getHeight(), img4.getHeight())) + offset;
